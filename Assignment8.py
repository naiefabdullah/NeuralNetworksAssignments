...
class Activation:
    ...
    def forward(self, Z):
        if self.type == "linear":
            ...
            e_Z = np.exp(Z - np.max(Z, axis=0, keepdims=True))  # softmax stabilization
    ...
    def backward(self, dA, Z):
        if self.type == "linear":
            ...
            s = self.forward(Z)  # Use the forward pass result
            dZ = s - dA
            return dZ
        else:
...
class Layer:
    ...
    def forward(self, A_prev, keep_prob=1):
        ...
        if keep_prob < 1:
            ...
            self.A /= keep_prob  # Scale the output to maintain the expected value
...
activation_types = ["relu", "relu", "softmax"]  # Define activation functions for each layer
...
# Train the network with L2 regularization and dropout using mini-batch approach
network.train(X, Y, learning_rate=0.001, num_iterations=5000, print_cost=True, lambd=0.1, keep_prob=0.8, mini_batch_size=64)
